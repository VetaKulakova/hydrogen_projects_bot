...
def start_kb():
    ...
    start_menu_kb.add(KeyboardButton("💧 Краткая справка о водороде"), KeyboardButton("📊 Аналитика")).add(KeyboardButton("🌐 Новости"), KeyboardButton("🧮 Водородный калькулятор"))
    return start_menu_kb

def calculations_kb():
    vpt_btn = InlineKeyboardButton('Массу водорода (зная P, V, T)', callback_data='calc button 1')
    tp_m_btn = InlineKeyboardButton('Объем водорода (при стандартных условиях)', callback_data='calc button 2')
    tp_v_btn = InlineKeyboardButton('Массу водорода (при стандартных условиях)', callback_data='calc button 3')
    calc_inline_kb = InlineKeyboardMarkup(row_width=1).add(vpt_btn, tp_m_btn, tp_v_btn)
    return calc_inline_kb

def statistic_kb():
    countries_btn = InlineKeyboardButton('Распределение проектов по странам', callback_data='stat_button_1')
    years_btn = InlineKeyboardButton('Ввод проектов по годам', callback_data='stat_button_2')
