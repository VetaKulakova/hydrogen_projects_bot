...
sorted_countries = sorted(countries_full_list.items(),
                          key=lambda x: x[1],
                          reverse=True)[:20]
sorted_countries = sorted_countries[-1::-1]
countries, projects = zip(*sorted_countries)

sorted_years = sorted(years_full_list.items(),
                      key=lambda x: x[1],
                      reverse=True)
...
def countries_plot(SAVE_PATH):
    ...
    for i, value in enumerate(projects):
        ax.text(i, value + 0.1, str(value),
                ha='center',  va='bottom', fontsize=12)
    ax.text(-1, 197, r'Источник: International Energy Agency, 2023',
            fontsize=11, color='#6A7880')
...
def years_plot(SAVE_PATH):
    ...
    colors = ['#56B9F2' if year < 2024
              else ('#1476C6' if year == 2024 else '#C4C4C4')
              for year in years
              ]
    ...
    ax.tick_params(axis='x', color='#D9D9D9')
    ax.tick_params(axis='y', color='#D9D9D9')
    plt.text(x=2030, y=240, s=r'Источник: International Energy Agency, 2023',
             fontsize=11, color='#6A7880')
    plt.xticks(years, rotation=60, fontsize=12)

    # Добавляем подписи на столбцах
    for i, value in enumerate(projects_y):
        plt.text(years[i], value + 0.1, str(value),
